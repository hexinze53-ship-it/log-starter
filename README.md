...
![alt text](https://img.shields.io/badge/Spring%20Boot-2.7.18-brightgreen.svg)

![alt text](https://img.shields.io/badge/Java-8%2B-blue.svg)

![alt text](https://img.shields.io/badge/License-MIT-orange.svg)
📖 项目简介 (Introduction)
本项目是一个为 Spring Boot 量身定制的无侵入式、高性能日志审计与链路追踪 Starter 组件。
抽离并重构了开源项目中的日志监控核心思想，摒弃了传统同步写库带来的极高 I/O 损耗。通过引入内存缓冲队列、双重触发批处理、优雅停机防丢以及 MDC 全链路追踪等极致优化，在保障主业务“零延迟”的前提下，实现了对用户操作轨迹的高可靠、结构化记录。
...
破局：引入 LinkedBlockingQueue 构建本地内存缓冲区。利用后台守护线程（Daemon） 作为消费者，采用 “容量（满 200 条）” 与 “时间（超时 5 秒）” 双重阈值触发机制，配合 Hibernate batch_size 将高频单次 Insert 转换为低频批量 Insert，数据库写入压力直降 90%。
...
(@Log AOP 环绕拦截) ──▶ 提取方法签名、入参、IP，从 MDC 提取 TraceId
      │
      ▼ (非阻塞 push)[LinkedBlockingQueue 内存铁皮箱]
      │
...
<dependencies>
    <dependency>
        ...
        <version>v1.0</version> <!-- 请替换为实际的 release 版本号 -->
    </dependency>
</dependencies>
2. 添加数据库配置
...
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/你的数据库?characterEncoding=utf8
    username: root
    password: password
  jpa:
    hibernate:
